...
LOG_BASE="/home/p4bs/splint_analysis/juliet-test-suite-c/debug/logs/${TIMESTAMP}"
OUTPUT_CSV_DIR="output-csv"
OUTPUT_CSV="${OUTPUT_CSV_DIR}/${TIMESTAMP}.csv"
SPLINT_FLAGS="-I${SUPPORT_DIR} +bounds +memchecks +charintliteral +posixlib"

# CREATE GOOD AND BAD TEST CASE LOGS
mkdir -p "${OUTPUT_CSV_DIR}"
mkdir -p "${LOGS_BASE}/bad" "${LOGS_BASE}/good"

echo "file,cwe,mode,status,warnings_raw,warning_count,key_warning" > "$OUTPUT_CSV"
...
find "$SUITE_DIR" -name "*.c" ! -path "*w32*" | while read file; do
    ...
    bad_output_raw=$(splint $SPLINT_FLAGS -DOMITGOOD "$file" 2>&1 | grep -v "^Splint ")
    echo "$bad_output_raw" > "$bad_log"
    bad_count=$(echo "$bad_output_raw" | grep -c "warning")
    bad_keyw=$(echo "$bad_output_raw" | grep -oP '(Possible out-of-bounds|Possible buffer overflow|null pointer dereference|Memory leak)' | head -1)
    bad_keyw=$(echo "$bad_keyw" | sed 's/"/""/g')
    echo "${filename},${cwe_dir},BAD,analyzed,\"${bad_log}\",${bad_count},\"${bad_keyw}\"" >> "$OUTPUT_CSV"

    # GOOD clause of test case
    good_output_raw=$(splint $SPLINT_FLAGS -DOMITBAD "$file" 2>&1 | grep -v "^Splint ")
    echo "$good_output_raw" > "$good_log"
    good_count=$(echo "$good_output_raw" | grep -c "warning")
    good_keyw=$(echo "$good_output_raw" | grep -oP '(Possible out-of-bounds|Possible buffer overflow|null pointer dereference|Memory leak)' | head -1)
    good_keyw=$(echo "$good_keyw" | sed 's/"/""/g')
    echo "${filename},${cwe_dir},GOOD,analyzed,\"${good_log}\",${good_count},\"${good_keyw}\"" >> "$OUTPUT_CSV"
done
